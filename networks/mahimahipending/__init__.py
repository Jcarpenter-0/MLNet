...
def SetupMahiMahiNode(mmShellsList, runDaemonServer=False, daemonPort=8081, dirOffset='./../',
                      inputDir: str = './daemon-proc-input/mm/') -> Tuple[networks.Node, str, str]:
    """
    ...
    if runDaemonServer and len(mmShellsList) <= 1:
        ...
        # add the proc daemon
        mmCommands.extend(
            apps.framework_Daemon_process.PrepareDaemonArgs(daemonServerWatchFilePath=inputDir, dirOffset=dirOffset))
    ...
    print('Framework: MM Node http://{}:{}/ - Proc {} {}'.format(ipAddress, daemonPort, finalmmProc.pid,
                                                                 finalmmProc.returncode))
...
def ParseMMLogFile(logFilePath: str, timeGrouping: int = 0) -> list:
    ...
    print('Packet Delay Stats: mean {} min {} max {} std {}'.format(np.mean(delays), np.min(delays), np.max(delays),
                                                                    np.std(delays)))
