...
class ReinforcementLearner:
    ...
    def ObserveEnvironment(self, clientLogJson, serverLogJson, networkLogJson):
        ...
        networkReward = networkLogJson['logs']['NetworkRewardVariable']
        serverReward = serverLogJson['logs']['ServerRewardVariable']
        clientReward = clientLogJson['logs']['ClientRewardVariable']


        # calculate reward
        reward = [networkLogJson['logs']['NetworkRewardVariable']
                  + serverLogJson['logs']['ServerRewardVariable']
                  + clientLogJson['logs']['ClientRewardVariable']
                  ]

        print('Reward:{}=|{}|{}|{}'.format(reward[0],networkReward,serverReward,clientReward))
    ...
    def TakeAction(self, actionIndex):
        ...
        if actionIndex == 0:
            # Starting Point
            self.Network.AvailableThroughput = 10
        elif actionIndex == 1:
            # Double
            self.Network.AvailableThroughput = 20
        elif actionIndex == 2:
            # Half
            self.Network.AvailableThroughput = 5
    ...
    def log(self):
        ...
        print(str(self.ActionHistory))
        print(str(self.RewardHistory))
        print(str(self.RewardByAction))

    # EDITABLE - Visualize/Graph/Human readable
    def report(self):
        pass

    # EDITABLE - Check result at end for correctness
    def validate(self):
        pass