...
for file in dataFiles:
    ...
    benchmarkName = file.split('/')[-1].split('.')[0].split('-')[6]

    combinedDF[benchmarkName] = df2['Reward']

colors = ['red', 'green', 'blue', 'orange', 'yellow','brown', 'purple']

combinedDF = combinedDF.drop(['lp', 'westwood', 'veno', 'bic'], axis=1)

ax = combinedDF.plot(style=['--', '--', '--', '--', '--', '--', '--'], color=colors, title='Network Congestion Control - MDP Benchmark', figsize=(8, 8))
...
ax = combinedDF.plot(style=['--'], color=colors, title='Network Congestion Control - MDP', figsize=(8, 8))
ax.set_ylabel('Reward')
ax.set_xlabel('Number of Actions (10s)')
